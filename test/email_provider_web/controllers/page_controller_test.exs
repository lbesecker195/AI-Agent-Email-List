defmodule EmailProviderWeb.PageControllerTest do
  ...
  describe "GET /app" do
    test "redirects to the console, so old links still land somewhere", %{conn: conn} do
      assert conn |> get("/app") |> redirected_to() == "/domains"
    end

    test "the landing page points at signup and login", %{conn: conn} do
  ...
  describe "GET /llms.txt" do
    ...
    test "the limits it publishes are the limits the service enforces", %{conn: conn} do
      ...
      on_exit(fn ->
        ...
        Application.delete_env(:email_provider, EmailProvider.Reputation)
      end)
